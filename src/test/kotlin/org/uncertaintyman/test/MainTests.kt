...
fun main(args: Array<String>) {
    ...
    socket.on("push_msg_event")  {
        println("push_msg_event:${it[0].toString()}")
        val data : JSONObject = JSON.parseObject(it[0].toString());
        if (it.size > 1 && it[1] is Ack) {
            val res: String = """
                {"msg_id": ${data.getJSONObject("data").get("msg_id")}}
            """;
            (it[1] as Ack).call(JSONObject.parseObject(res))
        }
